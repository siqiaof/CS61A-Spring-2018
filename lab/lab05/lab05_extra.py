...
def construct_sent(word, table):
    ...
    while word not in ['.', '!', '?']:
        "*** YOUR CODE HERE ***"
        result = result + word + ' '
        if table[word][0] not in ['.', '!', '?']:
            result = result + table[word][0] + ' '
            word = random.choice(list(table.keys()))
        else:
            word = table[word][0]
...
def prune_leaves(t, vals):
    ...
    if is_leaf(t):
        ...
        return tree(label(t),[prune_leaves(branch, vals) for branch in branches(t) if not(is_leaf(branch) and (label(branch) in vals))])
...
def add_trees(t1, t2):
    ...
    if is_leaf(t1):
        ...
        if len(branches(t1)) > len(branches(t2)):
            for i in range(len(branches(t2))):
                newbranches += [add_trees(branches(t1)[i],branches(t2)[i])]
            newbranches += branches(t1)[len(branches(t2)):]
        elif len(branches(t1)) < len(branches(t2)):
            for i in range(len(branches(t1))):
                newbranches += [add_trees(branches(t1)[i],branches(t2)[i])]
            newbranches += branches(t2)[len(branches(t1)):]
        else:
            for i in range(len(branches(t1))):
                newbranches += [add_trees(branches(t1)[i],branches(t2)[i])]
