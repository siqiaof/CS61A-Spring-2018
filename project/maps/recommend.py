...
def find_centroid(cluster):
    ...
    return [mean([x for x, y in locations]), mean([y for x, y in locations])]
    # END Question 5
...
def rate_all(user, restaurants, feature_fns):
    ...
    # BEGIN Question 9
    "*** YOUR CODE HERE ***"
    review = [x for x in restaurants if x in reviewed]
    unreview = [x for x in restaurants if x not in reviewed]
    ratings = {}
    for i in review:
        ratings[restaurant_name(i)] = review_rating(user_reviews(user)[restaurant_name(i)])
    for i in unreview:
        ratings[restaurant_name(i)] = predictor(i)
    return ratings
    # END Question 9
